...
/// Fetch Fleet Drivers
class FetchedFleetDrivers {
  static Future<ModelSeparateDriverList> fetchDrivers({
    String? query,
    int limit = 20,
  }) async {
    final odooClient = await OdooSessionManager.callKwWithCompany;
    final domain = (query == null || query.trim().isEmpty)
        ? []
        : [
            '|',
    ...
    final List<dynamic> response = await odooClient({
      ...
      'kwargs': {
        'limit': limit,
        'fields': ['id', 'complete_name', 'email', 'phone', 'avatar_128'],
      },
    });
    return ModelSeparateDriverList.fromJson({
...
class ModelSeparateDriverList {
  final int length;
  final List<Record> records;

  ModelSeparateDriverList({required this.length, required this.records});

  factory ModelSeparateDriverList.fromJson(Map<String, dynamic> json) {
    return ModelSeparateDriverList(
